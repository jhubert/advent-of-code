...
CRATE_MOVER_9000 = -> (crates, count, from, to) { crates[to].concat(crates[from].pop(count.to_i).reverse) }
...
moves_data.split("\n").each do |move|
  ...
  CRATE_MOVER_9000.call(crates, count, from, to)
end

last_crates_in_each_stack = crates.collect { |_number, crate_name| crate_name.last }.join

puts "The Crate Mover 9000 will result in the following top crates in each stack: #{last_crates_in_each_stack}"
